...
const Category = () => {
            ...
            try {
                ...
                if (response.status === 200 && response.data.success) {
                    const productsData = response.data.data.filter(product => {
                        return product.category && product.category.pid === 83;
                    }).map(product => {
	...
	const [categories, setCategories] = useState([]);
	  ...
	  useEffect(() => {
        axios.get(`${URL}/category/a51dfc6e-9706-11ee-b6c4-02120a000912/subcategory?only_main=true`)
            .then(response => {
                if (response.status === 200 && response.data.success) {
  ...
  const toggle = () => setModal(!modal);
	return (
		<Fragment>
			<Breadcrumb title="Erkek Ürünleri" parent="Ürünler" />
